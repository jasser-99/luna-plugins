...
let statusDisplayTypeOverride: number | undefined;
export const setStatusDisplayTypeOverride = (value?: number) => {
	statusDisplayTypeOverride = value;
};
...
export const updateActivity = asyncDebounce(async (mediaItem?: MediaItem) => {
	...
	// Status text
	activity.statusDisplayType = statusDisplayTypeOverride ?? settings.status;
